...
Agregar prop `pageHeading?: string` y renderizar un `<h1>` visualmente oculto dentro del `<body>`, para que cada página declare su H1 sin romper el diseño fullpage.
...
| `europa-danubio.astro` | `Crucero por el Danubio — Fotografía de viajes` | `Galería fotográfica del Crucero por el Danubio: Passau, Linz, Viena, Bratislava, Budapest, Salzburgo y más. Fotografías de Orlando Moure.` |
...
Además agregar `canonical` y `ogImage` en cada página:
```astro
<Layout
...
Schema mínimo recomendado por página:
```js
const jsonLd = {
...
Usar `captions[img]?.title` cuando esté disponible en lugar del nombre de archivo:

```astro
alt={captions[img]?.title ?? `${sectionName} — ${img.replace(/\.\w+$/, '').replace(/-/g, ' ')}`}
```

Línea de referencia: `index.astro:135`.
...
| `src/pages/index.astro` | Agregar canonical, ogImage, pageHeading, jsonLd |
...
1. `npm run build` — confirmar que genera `dist/sitemap-index.xml` con las 9 URLs
2. Abrir `dist/robots.txt` — verificar referencia al sitemap
3. Revisar cada `dist/*.html` y verificar `<title>`, `<meta name="description">`, `<link rel="canonical">`, `<h1>`, `<script type="application/ld+json">`
4. `npm run preview` — confirmar que el H1 oculto no rompe el layout visualmente
